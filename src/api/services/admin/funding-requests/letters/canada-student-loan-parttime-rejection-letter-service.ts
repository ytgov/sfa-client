...
import CanadaStudentLoanParttimeRejectionTemplateSerializer from "@/serializers/funding-requests/letters/canada-student-loan-parttime-rejection-template-serializer";
...
export default class CanadaStudentLoanRejectionLetterService extends FundingRequestsLettersBaseService {
  getTemplatePath(): string {
    return TemplatePaths.CANADA_STUDENT_LOAN_PARTTIME_REJECTION;
  }

  serializeForTemplate() {
    return CanadaStudentLoanParttimeRejectionTemplateSerializer.prepare({
  ...
  buildFileName({ format }: { format: string }): string {
    ...
    return `CSLPT_Rejection_Letter_${lastName}_${firstName}.${format}`;
  }
}
