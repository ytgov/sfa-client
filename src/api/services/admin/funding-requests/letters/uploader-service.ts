...
export default class Uploader {
  ...
  constructor({ currentUser, fundingRequest }: { currentUser: User; fundingRequest: FundingRequest }) {
    this.#documentService = new DocumentService();
    this.#currentUser = currentUser;
    this.#fundingRequest = fundingRequest;
  }

  async upload(buffer: Buffer, filename: string, visibleInPortal = true): Promise<FileReference> {
