...
exports.up = async function (_: Knex) {
  ...
  for (let sub of subs) {
    ...
    let lastTravelYear = app.residency.lastReturnDate ? app.residency.lastReturnDate.split("/")[0] : null;
    let lastTravelMonth = app.residency.lastReturnDate ? app.residency.lastReturnDate.split("/")[1] : null;
    ...
    let studentApps = await db("application")
      .where({ studentId: sub.studentId, academicYearId: 2023 });
