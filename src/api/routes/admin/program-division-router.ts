...
export const programDivisionRouter = express.Router();

programDivisionRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.program_division")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.program_division.id',
...
programDivisionRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.program_division")
                .select('description')
            ...
            const resInsert = await db("sfa.program_division")
...
programDivisionRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.program_division")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
programDivisionRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
programDivisionRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.program_division")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.program_division")
                .where({ id: id })
                .del();
