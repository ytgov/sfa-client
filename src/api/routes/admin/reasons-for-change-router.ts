...
export const reasonsForChangeRouter = express.Router();

reasonsForChangeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.change_reason")
            .select(
                'sfa.change_reason.id',
                'sfa.change_reason.description',
                'sfa.change_reason.is_active',
            )
            .orderBy('sfa.change_reason.description');
...
reasonsForChangeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.change_reason")
                .select('description')
            ...
            const resInsert = await db("sfa.change_reason")
...
reasonsForChangeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.change_reason")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
reasonsForChangeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
reasonsForChangeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.change_reason")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.change_reason")
                .where({ id: id })
                .del();
