...
export const applicationTypeRouter = express.Router();

applicationTypeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.application_type")
            .select(
                'sfa.application_type.id',
                'sfa.application_type.description',
                'sfa.application_type.is_active',
            )
            .orderBy('sfa.application_type.description');
...
applicationTypeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.application_type")
                .select('description')
            ...
            const resInsert = await db("sfa.application_type")
...
applicationTypeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.application_type")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
applicationTypeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
applicationTypeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.application_type")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.application_type")
                .where({ id: id })
                .del();
