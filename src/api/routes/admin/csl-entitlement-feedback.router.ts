...
cslEntitlementFeedbackRouter.post(
  "/:FILE_NAME",
  ...
  async (req: Request, res: Response) => {
    ...
    async function getErrorDesc(id: Number) {
      let result;
      try {
        result = await db("sfa.entitlement_error_codes")
          .select(db.raw("description as description"))
          .where("id", "=", 77)
          .first();
      } catch (error: any) {
        return "-1";
    ...
    try {
      ...
      if (filename) {
        ...
        if (filename.substring(0, 1).match(/[A-Z]/) && filename.length > 5) {
          ...
          if (header.substring(0, 1).toUpperCase() !== "H") {
            ...
            if (isNaN(vSeqNum)) {
              return res.json({
                flag: 0,
                data: "Invalid sequence number in header record: " + header.substring(1, 10),
              });
            }
            ...
            if (vProvince !== "YT") {
              ...
              if (vOriginator !== "222") {
                return res.json({
                  flag: 0,
                  data: "File has not been provided by the service provider (222) but by: " + vOriginator,
                });
              } else {
                ...
                if (vCreateDate === "Invalid date") {
                  return res.json({
                    flag: 0,
                    data: "Invalid creation date/time in header record: " + header.substring(15, 23),
                  });
                }
          ...
          const selectCountDisbursement = await db("sfa.disbursement")
          ...
          if ((vCount === null || vCount === undefined ? 0 : vCount) !== 0) {
            const selectCountAndDateDisbursement = await db("sfa.disbursement")
              .select(db.raw("ecert_sent_date AS date, COUNT (*) AS count"))
              .where("csl_cert_seq_number", "=", vSeqNum)
              .andWhere(db.raw("disbursement_type_id IN (4, 3)"))
              .andWhereNot(db.raw("ecert_sent_date IS NULL"))
              .andWhere("issue_date", ">", "2013-01-01")
          ...
          for (let i = 1; i < linesContent.length; i++) {
            ...
            if (currentLine.substring(0, 1) === "D") {
              ...
              if (vCertNumber === "") {
                ...
                vDisbursementId = await db("sfa.disbursement")
                  .select("id")
                ...
                if (!vDisbursementId || !vDisbursementId.id) {
                  return res.json({
                    flag: 2,
                    data:
                      "No records found with transaction number: " +
                      vCertNumber +
                      ", CSL Cert Number: " +
                      vSeqNum +
                      ", or disbursed amount: " +
                ...
                try {
                  result = await db.raw(
                    "DECLARE @is_resend_p VARCHAR(255), @error_id_p INT; " +
                      `EXEC sfa.sp_check_error_status '${vErrorCode1.replaceAll("\r", "")}', ${
                        vDisbursementId.id
                      }, @is_resend_p OUTPUT, @error_id_p OUTPUT; ` +
              ...
              if (vErrorCode2.length > 1) {
                ...
                try {
                  result = await db.raw(
                    "DECLARE @is_resend_p VARCHAR(255), @error_id_p INT; " +
                      `EXEC sfa.sp_check_error_status '${vErrorCode2.replaceAll("\r", "")}', ${
                        vDisbursementId.id
                      }, @is_resend_p OUTPUT, @error_id_p OUTPUT; ` +
                      "SELECT @is_resend_p as is_resend_p, @error_id_p as error_id_p;"
                  );
              ...
              if (vErrorCode3.length > 1) {
                ...
                try {
                  result = await db.raw(
                    "DECLARE @is_resend_p VARCHAR(255), @error_id_p INT; " +
                      `EXEC sfa.sp_check_error_status '${vErrorCode3.replaceAll("\r", "")}', ${
                        vDisbursementId.id
                      }, @is_resend_p OUTPUT, @error_id_p OUTPUT; ` +
                      "SELECT @is_resend_p as is_resend_p, @error_id_p as error_id_p;"
                  );
              ...
              if (vErrorCode4.length > 1) {
                ...
                try {
                  result = await db.raw(
                    "DECLARE @is_resend_p VARCHAR(255), @error_id_p INT; " +
                      `EXEC sfa.sp_check_error_status '${vErrorCode4.replaceAll("\r", "")}', ${
                        vDisbursementId.id
                      }, @is_resend_p OUTPUT, @error_id_p OUTPUT; ` +
                      "SELECT @is_resend_p as is_resend_p, @error_id_p as error_id_p;"
                  );
              ...
              if (vErrorCode5.length > 1) {
                ...
                try {
                  result = await db.raw(
                    "DECLARE @is_resend_p VARCHAR(255), @error_id_p INT; " +
                      `EXEC sfa.sp_check_error_status '${vErrorCode5.replaceAll("\r", "")}', ${
                        vDisbursementId.id
                      }, @is_resend_p OUTPUT, @error_id_p OUTPUT; ` +
                      "SELECT @is_resend_p as is_resend_p, @error_id_p as error_id_p;"
                  );
              ...
              if (isNaN(vAcceptedTot)) {
                return res.json({
                  flag: 0,
                  data: "Trailer accepted count is non numeric" + currentLine.substring(1, 10).replaceAll(" ", ""),
                });
              }
              ...
              if (isNaN(vRejectedTot)) {
                return res.json({
                  flag: 0,
                  data: "Trailer rejected count is non numeric: " + currentLine.substring(10, 19).replaceAll(" ", ""),
                });
              }
              if (vRejectedTot !== vRejectedCount) {
                return res.json({
                  flag: 0,
                  data:
                    "Trailer rejected count does not equal total rejected records: Trailer - " +
                    Number(currentLine.substring(43, 52).replaceAll(" ", "") + " Rejected Count - " + vRejectedCount),
                });
              }
              ...
              if (isNaN(vPaperTot)) {
                return res.json({
                  flag: 0,
                  data:
                    "Trailer paper enrollment count is non numeric: " + currentLine.substring(19, 28).replace(" ", ""),
                });
              }
              ...
              if (vTotal !== vCount && vCount !== vRejectedTot) {
                return res.json({
                  flag: 0,
                  data:
                    "Trailer accepted/rejected/paper enrollment count does not equal total records sent: Trailer - " +
                    vTotal +
                    " Record sent - " +
                    vCount,
                });
              ...
              if (isNaN(Number(currentLine.substring(52, 67).replaceAll(" ", "")))) {
                return res.json({
                  flag: 0,
                  data: "Trailer SIN hash is non numeric : " + currentLine.substring(52, 67).replaceAll(" ", ""),
                });
              }
              ...
              if (Number(currentLine.substring(52, 67).replaceAll(" ", "")) !== vTotalSin) {
                return res.json({
                  flag: 0,
                  data:
                    "Trailer SIN hash total does not equal total of detail records: Trailer - " +
                    Number(currentLine.substring(52, 67).replaceAll(" ", "")) +
                    " Record SIN - " +
                    vTotalSin +
                    ". SIN non-numeric count was: " +
                    vNonNumericSin,
                });
              }
            } else {
          ...
          responseRead = {
            flag: 1,
            data:
              "CSL ECERT response read complete. " +
              vAcceptedTot +
              " accepted records processed, " +
              vRejectedTot +
              " rejected records processed and " +
              vPaperTot +
              " paper enrollment records processed.",
            date: vEcertSentDate,
            seq: vSeqNum,
          };
        } else {
        ...
        try {
          const execSPDisbursementBySeq = await db.raw(
            `EXEC sfa.sp_update_disbursement_by_seq ${
              vCreateDate ? `'${moment(vCreateDate, "YYYYMMDD").format("YYYY-MM-DD")}'` : null
            }, ${vSeqNum}, ${vEcertSentDate ? `'${moment(vEcertSentDate, "YYYYMMDD").format("YYYY-MM-DD")}'` : null}`
          );
        } catch (error: any) {
          return res.json({ flag: 0, data: "Something went wrong, please contact the administrator. " });
        }
        ...
        if (count === null || count === 0) {
          ...
          responseRead.data =
            responseRead.data + " CSL ECERT response update complete. " + vCount + " records processed.";
          ...
          const queryPDF = await db("sfa.ecert_import AS ei")
            .select(
              ...
              db.raw("CASE WHEN ei.is_resend_flg = 'Yes' THEN 'Rejected' ELSE 'Warning' END AS ecert_status"),
              db.raw(
                "ei.error_message + ISNULL((SELECT CONCAT(FIRST_NAME, ' ', LAST_NAME) FROM sfa.person WHERE SIN = ei.SIN), ' No Student Match') + ISNULL((SELECT DISTINCT transaction_number FROM sfa.disbursement WHERE transaction_number = ei.certificate_number AND ecert_sent_date = ei.ecert_sent_date), ' No CSL Certificate Match') AS error_message"
              ),
              db.raw(
                `ISNULL((SELECT CONCAT(FIRST_NAME, ' ', LAST_NAME) FROM sfa.person WHERE SIN = ei.SIN), '') AS student_name`
              )
            )
            .orderBy("ei.sin");
