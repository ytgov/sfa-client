...
export const prestudyEmploymentRouter = express.Router();

prestudyEmploymentRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.prestudy_employment_status")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.prestudy_employment_status.id',
                'sfa.prestudy_employment_status.description',
                'sfa.prestudy_employment_status.is_active',
            )
            .orderBy('sfa.prestudy_employment_status.description');
...
prestudyEmploymentRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.prestudy_employment_status")
                .select('description')
            ...
            const resInsert = await db("sfa.prestudy_employment_status")
...
prestudyEmploymentRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.prestudy_employment_status")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
prestudyEmploymentRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
prestudyEmploymentRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.prestudy_employment_status")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.prestudy_employment_status")
                .where({ id: id })
                .del();
