...
export const disbursementTypeRouter = express.Router();

disbursementTypeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.disbursement_type")
            .select(
                'sfa.disbursement_type.id',
                'sfa.disbursement_type.description',
                'sfa.disbursement_type.is_active',
            )
            .orderBy('sfa.disbursement_type.description');
...
disbursementTypeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.disbursement_type")
                .select('description')
            ...
            const resInsert = await db("sfa.disbursement_type")
...
disbursementTypeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.disbursement_type")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
disbursementTypeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
disbursementTypeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.disbursement_type")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.disbursement_type")
                .where({ id: id })
                .del();
