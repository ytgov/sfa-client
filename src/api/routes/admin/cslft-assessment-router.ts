...
assessmentCslftRouter.get(
  "/funding_request/:id/assessment/create",
  [param("id").isInt().notEmpty()],
  ReturnValidationErrors,
  async (req: Request, res: Response) => {
    const assessmentCslftRepo = new AssessmentCslftRepository(db);
...
assessmentCslftRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (newRow) {
        return res.json({
          data: { id: newRow.data?.id },
          messages: [{ text: "Assessment created", variant: "success" }],
        });
      }
...
assessmentCslftRouter.put(
  "/:id",
  [param("id").isInt().notEmpty(), body("payload").notEmpty()],
  ReturnValidationErrors,
  async (req: Request, res: Response) => {
    const { ...assessment } = req.body.payload.data;
    const disbursements = req.body.payload.disbursements;
    ...
    try {
      ...
      if (updateRow) {
        return res.json({
          data: { id: updateRow.data?.id },
          messages: [{ text: "Assessment updated", variant: "success" }],
        });
      }
...
assessmentCslftRouter.post(
  "/academic-year/:academic_year_id/person/:person_id/parentweeklycontrib",
  [param("academic_year_id").isInt().notEmpty(), param("person_id").isInt().notEmpty(), body("assessment").notEmpty()],
  ReturnValidationErrors,
  async (req: Request, res: Response) => {
    const assessmentCslftRepo = new AssessmentCslftRepository(db);
    ...
    try {
      if (academic_year_id && person_id) {
        result = await assessmentCslftRepo.calculateParentWeeklyContrib(
          parseInt(person_id),
...
assessmentCslftRouter.post(
  ...
  [param("funding_request_id").isInt().notEmpty(), body("assessment").notEmpty()],
  ReturnValidationErrors,
  async (req: Request, res: Response) => {
    const assessmentCslftRepo = new AssessmentCslftRepository(db);
    const { ...assessment } = req.body.assessment;
    ...
    try {
      if (funding_request_id) {
        results = await assessmentCslftRepo.executeRecalc(
...
assessmentCslftRouter.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      if (funding_request_id) {
        const disburse = await assessmentCslftRepo.executeDisburse(
          parseInt(funding_request_id),
          assessment as AssessmentDTO,
          disbursements
        );
...
assessmentCslftRouter.get(
  "/application/:application_id/expenses/uncapped/:period_id",
  [param("application_id").isInt().notEmpty(), param("period_id").isInt().notEmpty()],
  ReturnValidationErrors,
  async (req: Request, res: Response) => {
...
assessmentCslftRouter.get(
  "/e-certificate/list/:assessment_id",
