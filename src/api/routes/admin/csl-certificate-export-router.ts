...
cslCertificateExportRouter.get(
  "/:FROM_DATE_P/:TO_DATE_P/:CSL_CERT_SEQ_P/:IS_PREVIEW",
  [
...
cslCertificateExportRouter.put(
  "/:FROM_DATE_P/:TO_DATE_P/:PREVIEW",
  [param("FROM_DATE_P").notEmpty(), param("TO_DATE_P").notEmpty(), param("PREVIEW")],
  //ReturnValidationErrors,
  async (req: Request, res: Response) => {
    const { filter = true } = req.query;
