...
agencyRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.agency")
            .where("is_active", filter !== 'false')
            .select(
                'sfa.agency.id',
                'sfa.agency.description',
                'sfa.agency.is_active',
            )
            .orderBy('sfa.agency.description');
...
agencyRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.agency")
                .select('description')
            ...
            const resInsert = await db("sfa.agency")
...
agencyRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
...
agencyRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
agencyRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.agency")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.agency")
                .where({ id: id })
                .del();
