...
export const accommodationTypeRouter = express.Router();

accommodationTypeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db({at: "sfa.accommodation_type"})
...
accommodationTypeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db({ at: "sfa.accommodation_type"})
            ...
            const resInsert = await db("sfa.accommodation_type")
...
accommodationTypeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.accommodation_type")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
accommodationTypeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
accommodationTypeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db({ at: "sfa.accommodation_type"})
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db({ at: "sfa.accommodation_type"})
                .where({ id: id })
                .del();
