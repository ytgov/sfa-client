...
export const cslClassificationRouter = express.Router();

cslClassificationRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.csl_classification")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.csl_classification.id',
                'sfa.csl_classification.description',
                'sfa.csl_classification.is_active',
            )
            .orderBy('sfa.csl_classification.description');
...
cslClassificationRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.csl_classification")
                .select('description')
            ...
            const resInsert = await db("sfa.csl_classification")
...
cslClassificationRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.csl_classification")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
cslClassificationRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
cslClassificationRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.csl_classification")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.csl_classification")
                .where({ id: id })
                .del();
