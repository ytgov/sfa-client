...
export const instructionTypeRouter = express.Router();

instructionTypeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.instruction_type")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.instruction_type.id',
                'sfa.instruction_type.description',
                'sfa.instruction_type.is_active',
            )
            .orderBy('sfa.instruction_type.description');
...
instructionTypeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.instruction_type")
                .select('description')
            ...
            const resInsert = await db("sfa.instruction_type")
...
instructionTypeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.instruction_type")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
instructionTypeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
instructionTypeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.instruction_type")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.instruction_type")
                .where({ id: id })
                .del();
