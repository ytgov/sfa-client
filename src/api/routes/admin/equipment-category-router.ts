...
export const equipmentCategoryRouter = express.Router();

equipmentCategoryRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.equipment_category")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.equipment_category.id',
                'sfa.equipment_category.description',
                'sfa.equipment_category.is_active',
            )
        .orderBy('sfa.equipment_category.description');
...
equipmentCategoryRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.equipment_category")
                .select('description')
            ...
            const resInsert = await db("sfa.equipment_category")
...
equipmentCategoryRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.equipment_category")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
equipmentCategoryRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
equipmentCategoryRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.equipment_category")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.equipment_category")
                .where({ id: id })
                .del();
