...
export const addressTypeRouter = express.Router();

addressTypeRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const addressTypes = await db("sfa.address_type")
            .select(
                'sfa.address_type.id',
                'sfa.address_type.description',
                'sfa.address_type.is_active',
            )
            .orderBy('sfa.address_type.description');

        if (addressTypes) {

            if (filter !== 'false') {
                return res.status(200).json({ success: true, data: addressTypes.filter(c => c.is_active), })
            } else {
                return res.status(200).json({ success: true, data: [...addressTypes], });
            }
...
addressTypeRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.address_type")
                .select('description')
            ...
            const resInsertAddress = await db("sfa.address_type")
                .insert({ description: trimDescription, is_active })
                .returning("*");

            if (resInsertAddress) {
                return res.status(201).json({ success: true, data: resInsertAddress, });
            } else {
...
addressTypeRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
...
addressTypeRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
addressTypeRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.address_type")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.address_type")
                .where({ id: id })
                .del();
