...
programRouter.get("/", async (req: Request, res: Response) => {
    ...
    try {
        const results = await db("sfa.program")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.program.id',
                'sfa.program.description',
                'sfa.program.education_level_id',
                'sfa.program.is_active',
            )
            .orderBy('sfa.program.description');
...
programRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.program")
                .select('description')
            ...
            const resInsert = await db("sfa.program")
...
programRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
...
programRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
programRouter.patch("/education-level/:id", [param("id").isInt().notEmpty(), body("study_field_id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { education_level_id = null } = req.body;

        db("sfa.program")
            .update({ education_level_id })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
                if (resp[0]?.id === Number(id)) {
...
programRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.program")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.program")
                .where({ id: id })
                .del();
