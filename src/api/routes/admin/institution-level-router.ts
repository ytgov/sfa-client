...
export const institutionLevelRouter = express.Router();

institutionLevelRouter.get("/", async (req: Request, res: Response) => {

    const { filter = true } = req.query;

    try {
        const results = await db("sfa.institution_level")
            .select(
                'sfa.institution_level.id',
                'sfa.institution_level.description',
                'sfa.institution_level.is_active',
            )
            .orderBy('sfa.institution_level.description');
...
institutionLevelRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.institution_level")
                .select('description')
            ...
            const resInsert = await db("sfa.institution_level")
...
institutionLevelRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    (req: Request, res: Response) => {

        const { id = null } = req.params;
        const { is_active = false } = req.body;

        db("sfa.institution_level")
            .update({ is_active })
            .where({ id })
            .returning("*")
            .then((resp: any) => {
...
institutionLevelRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
institutionLevelRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.institution_level")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.institution_level")
                .where({ id: id })
                .del();
