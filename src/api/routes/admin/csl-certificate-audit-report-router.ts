...
const RELEVANT_REQUEST_TYPES = [4, 5, 28, 29, 31, 32, 33, 35];
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      let notSentData = await db.raw(`SELECT d.transaction_number cert_num, m.id msfaa_num, m.received_date,
        ...
        ORDER BY CASE WHEN d.transaction_number IS NULL THEN 2 ELSE 1 END, 1, CASE WHEN rt.id IN (${RELEVANT_LOAN_TYPES.join(",")}) THEN 1 ELSE 2 END, due_date, rt.id`);
