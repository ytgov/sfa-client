...
attendanceRouter.get("/", async (req: Request, res: Response) => {
    ...
    try {
        const results = await db("sfa.attendance")
        .where("is_active", filter !== 'false') 
        .select(
                'sfa.attendance.id',
                'sfa.attendance.description',
                'sfa.attendance.is_active',
            )
        .orderBy('sfa.attendance.description');
...
attendanceRouter.post("/", body('is_active').isBoolean(), body('description').isString(),
    ...
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verify = await db("sfa.attendance")
                .select('description')
            ...
            const resInsert = await db("sfa.attendance")
...
attendanceRouter.patch("/status/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
...
attendanceRouter.patch("/description/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
...
attendanceRouter.delete("/:id", [param("id").isInt().notEmpty()], ReturnValidationErrors,
    async (req: Request, res: Response) => {
        ...
        try {
            ...
            const verifyRecord: any = await db("sfa.attendance")
                .where({ id: id })
                .first();
            ...
            const deleteRecord: any = await db("sfa.attendance")
                .where({ id: id })
                .del();
