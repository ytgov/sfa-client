<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Application Type', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(APPLICATION_TYPE + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setApplicationTypes", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(APPLICATION_TYPE + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(APPLICATION_TYPE + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setApplicationTypes", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "applicationTypes"]),
    filteredList: function () {
      let l = _.clone(this.applicationTypes);
  ...
  async created() {
    await store.dispatch("setApplicationTypes", false);
  },
  components: {
    ModalConfirm,
  }
