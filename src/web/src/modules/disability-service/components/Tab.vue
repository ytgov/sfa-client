<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Disability Service', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(DISABILITY_SERVICE + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setDisabilityServices", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(DISABILITY_SERVICE + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(DISABILITY_SERVICE + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setDisabilityServices", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "disabilityServices"]),
    filteredList: function () {
      let l = _.clone(this.disabilityServices);
  ...
  async created() {
    await store.dispatch("setDisabilityServices", false);
  },
  components: {
    ModalConfirm,
  }
