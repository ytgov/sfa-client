<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Institution Level', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(INSTITUTION_LEVEL + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setInstitutionLevels", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(INSTITUTION_LEVEL + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(INSTITUTION_LEVEL + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setInstitutionLevels", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "institutionLevels"]),
    filteredList: function () {
      let l = _.clone(this.institutionLevels);
  ...
  async created() {
    await store.dispatch("setInstitutionLevels", false);
  },
  components: {
    ModalConfirm,
  }
