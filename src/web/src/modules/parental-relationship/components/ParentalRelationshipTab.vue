<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Parental Relationship', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(PARENTAL_RELATIONSHIP + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setParentalRelationships", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(PARENTAL_RELATIONSHIP + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(PARENTAL_RELATIONSHIP + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setParentalRelationships", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "parentalRelationships"]),
    filteredList: function () {
      let l = _.clone(this.parentalRelationships);
  ...
  async created() {
    await store.dispatch("setParentalRelationships", false);
  },
  components: {
    ModalConfirm,
  }
