<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Reasons for change', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(REASONS_FOR_CHANGE + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
    ...
    changeDescription(id, description) {
      axios.patch(REASONS_FOR_CHANGE + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(REASONS_FOR_CHANGE + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setReasonsForChange", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "reasonsforChangeList"]),
    filteredList: function () {
      let l = _.clone(this.reasonsforChangeList);
  ...
  async created() {
    await store.dispatch("setReasonsForChange", false);
  },
  components: {
    ModalConfirm,
  }
