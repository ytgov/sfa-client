<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Disbursement type', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(DISBURSEMENT_TYPE + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setDisbursementTypes", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(DISBURSEMENT_TYPE + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(DISBURSEMENT_TYPE + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setDisbursementTypes", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "disbursementTypes"]),
    filteredList: function () {
      let l = _.clone(this.disbursementTypes);
  ...
  async created() {
    await store.dispatch("setDisbursementTypes", false);
  },
  components: {
    ModalConfirm,
  }
