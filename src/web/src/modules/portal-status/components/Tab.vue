<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Portal Status', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(PORTAL_STATUS + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setPortalStatus", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(PORTAL_STATUS + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(PORTAL_STATUS + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setPortalStatus", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "portalStatus"]),
    filteredList: function () {
      let l = _.clone(this.portalStatus);
  ...
  async created() {
    await store.dispatch("setPortalStatus", false);
  },
  components: {
    ModalConfirm,
  }
