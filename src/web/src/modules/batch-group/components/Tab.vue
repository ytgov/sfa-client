<template>
  <div>
    ...
    <v-data-table :search="search" :headers="[
      { text: 'Batch Group', value: 'description', filterable: true, },
...
export default {
  ...
  methods: {
    ...
    changeStatus(id, isActive) {
      axios.patch(BATCH_GROUP + `/status/${id}`, { is_active: isActive })
        .then((resp) => {
        ...
        .finally(() => {
          store.dispatch("setBatchGroups", false);
        });
    },
    changeDescription(id, description) {
      axios.patch(BATCH_GROUP + `/description/${id}`, { description: description })
        .then((resp) => {
          resp?.data?.wasUpdated ?
    ...
    deleteRecord() {
      axios.delete(BATCH_GROUP + `/${this.idToDelete}`)
        .then((resp) => {
          resp?.data?.wasDelete && store.dispatch("setBatchGroups", false);;
  ...
  computed: {
    ...mapGetters(["search", "activeOnlyFilter", "batchGroups"]),
    filteredList: function () {
      let l = _.clone(this.batchGroups);
  ...
  async created() {
    await store.dispatch("setBatchGroups", false);
  },
  components: {
    ModalConfirm,
  }
