<template>
  <v-row justify="center">
    <v-dialog v-model="dialog" persistent max-width="600px">
      <v-card>
        <v-card-title>
          <span class="text-h5">Add Indigenous Learner</span>
        </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
...
export default {
  ...
  methods: {
    async addIndigenousLearner() {
      try {
        const resIndigenousLearner = await axios.post(INDIGENOUS_LERNER, {
          is_active: this.is_active,
          description: this.describe,
        });

        resIndigenousLearner?.data?.success ?
