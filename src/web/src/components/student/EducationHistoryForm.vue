  ...
  <div class="home">
    <v-card class="default mb-5">
      ...
      <v-card-text>
        ...
        <div class="row">
          <div class="col-md-12">
            <v-autocomplete outlined dense background-color="white" hide-details label="High school"
              v-model="student.high_school_id" :items="highSchools" item-text="name" item-value="id"
              @change="doSaveEducation('high_school_id', student.high_school_id, 'studentInfo', student.id)"></v-autocomplete>
          </div>
        </div>
        ...
        <div class="row">
          ...
          <div class="col-md-4">
            <v-text-field disabled outlined dense background-color="white" hide-details label="City"
            :value="cities.find(c => Number(c.id) === Number(student.high_school_info.city_id))?.description"  
            item-value="id"></v-text-field>
          </div>
          <div class="col-md-4">
              <v-text-field disabled outlined dense background-color="white" hide-details label="Province"
            :value="provinces.find(c => Number(c.id) === Number(student.high_school_info.province_id))?.description"  
            item-value="id"></v-text-field>
          </div>
          <div class="col-md-4">
              <v-text-field disabled outlined dense background-color="white" hide-details label="Country"
        ...
        <div class="row">
          <div class="col-md-2">
            <v-select outlined dense background-color="white" hide-details label="Year"
              v-model="student.high_school_left_year" :items="yearOptions"
              @change="doSaveEducation('high_school_left_year', student.high_school_left_year, 'studentInfo', student.id)"></v-select>
          </div>
          <div class="col-md-2">
            <v-select outlined dense background-color="white" hide-details label="Month"
              v-model="student.high_school_left_month" :items="monthOptions"
              @change="doSaveEducation('high_school_left_month', student.high_school_left_month, 'studentInfo', student.id)"></v-select>
          </div>
          <div class="col-md-2">
            <v-select outlined dense background-color="white" hide-details label="Last Grade Completed"
              v-model="student.high_school_final_grade" :items="gradeOptions"
              @change="doSaveEducation('high_school_final_grade', student.high_school_final_grade, 'studentInfo', student.id)"></v-select>
          </div>
          <div class="col-md-6">
            <v-select outlined dense background-color="white" hide-details label="Highest education level"
              v-model="student.education_level_id" :items="educationLevels" item-value="id" item-text="description"
              @change="doSaveEducation('education_level_id', student.education_level_id, 'studentInfo', student.id)"></v-select>
          </div>
        </div>

        <vue-editable-grid class="my-grid-class" ref="grid" id="mygrid" :column-defs="columnDefs"
          :row-data="student.residences" row-data-key="shipmentId" :enable-filters="false" :multiple-selection="false"
          @cell-updated="cellUpdated" @link-clicked="linkClicked">
        </vue-editable-grid>
          ...
          <div class="row" v-for="education, index of student?.education_info" :key="index">
            ...
            <div class="col-md">
              <v-select :disabled="isAdding" outlined dense background-color="white" hide-details label="From Year"
                ...
                :items="yearOptions"
                v-model="education.from_year" @keypress="validate.isNumber($event)">
              </v-select>
            </div>
            <div class="col-md">
              <v-select :disabled="isAdding" outlined dense background-color="white" hide-details label="From Month"
                ...
                :items="monthOptions"
                v-model="education.from_month" @keypress="validate.isNumber($event)">
            ...
            <div class="col-md">
              <v-select :disabled="isAdding" outlined dense background-color="white" hide-details label="To Year"
                ...
                :items="yearOptions"
                v-model="education.to_year" @keypress="validate.isNumber($event)">
            ...
            <div class="col-md">
              <v-select :disabled="isAdding" outlined dense background-color="white" hide-details label="To Month"
                ...
                :items="monthOptions"
                v-model="education.to_month" @keypress="validate.isNumber($event)">
        ...
        <div class="row">
          <div class="col-md-3">
            <v-autocomplete outlined dense background-color="white" hide-details label="Institution"
              v-model="newRegister.institution_campus_id" :items="filteredList" item-text="name"
              item-value="id"></v-autocomplete>
          </div>
          <div class="col-md-3">
            <v-autocomplete outlined dense background-color="white" hide-details label="Study Area"
              v-model="newRegister.study_area_id" :items="studyAreaOptions" item-text="description"
              item-value="id"></v-autocomplete>
          </div>
          <div class="col-md">
            <v-select outlined dense background-color="white" hide-details label="From Year"
              :items="yearOptions"
              v-model="newRegister.from_year" @keypress="validate.isNumber($event)">
            </v-select>
          </div>
          <div class="col-md">
            <v-select outlined dense background-color="white" hide-details label="From Month"
              :items="monthOptions"
              v-model="newRegister.from_month" @keypress="validate.isNumber($event)">

            </v-select>
          </div>
          <div class="col-md">
            <v-select outlined dense background-color="white" hide-details label="To Year"
              :items="yearOptions"
              v-model="newRegister.to_year" @keypress="validate.isNumber($event)">

            </v-select>
          </div>
          <div class="col-md">
            <v-select outlined dense background-color="white" hide-details label="To Month"
              :items="monthOptions"
              v-model="newRegister.to_month" @keypress="validate.isNumber($event)">
...
import { EDUCATION_LEVEL_URL, HIGH_SCHOOOL_URL, CITY_URL, COUNTRY_URL, PROVINCE_URL, INSTITUTION_URL, STUDY_AREA } from "../../urls";
import { mapGetters, mapState } from 'vuex';
import validator from "@/validator";

export default {
  computed: {
    ...mapGetters(["educationLevels", "highSchools", "cities", "provinces", "countries", "monthOptions", "yearOptions"]),
