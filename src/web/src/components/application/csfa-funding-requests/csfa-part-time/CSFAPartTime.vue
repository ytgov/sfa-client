    ...
    <v-card class="default row mb-5" :disabled="!checkCSFAPartTimeRequest">
      ...
      <div class="col-md-10 my-n2">
        <h3 class="text-subtitle-1 mt-1">
          Student is attending in eligible program at a designated post-secondary institution
        </h3>
      </div>
      <!--  -->
      ...
      <div class="col-md-12">
        <div class="row" v-for="(course, index) in application.courses_enrolled" :key="index">
          ...
          <div class="col-md-1 mt-11 d-md-flex justify-center">
            ...
            <v-btn :disabled="showAdd" color="error" x-small fab class="my-0 ml-1" @click="removeCourse(course.id)">
        ...
        <div class="row" v-if="showAdd || !application?.courses_enrolled?.length">
          <div class="col-md-6 pr-1">
            <h3 class="text-subtitle-1 text-center font-weight-bold">Course Description</h3>
            <v-text-field outlined dense background-color="white" hide-details v-model="newRecord.description">
            </v-text-field>
          </div>
          <div class="col-md-2 px-1">
            <h3 class="text-subtitle-1 text-center font-weight-bold">Course Code</h3>
            <v-text-field outlined dense background-color="white" hide-details v-model="newRecord.course_code">
            </v-text-field>
          </div>
          <div class="col-md-3 px-1">
            <h3 class="text-subtitle-1 text-center font-weight-bold">Instruction Type</h3>
          ...
          <div class="col-md-1 mt-11 d-md-flex justify-center">
            <v-btn color="success" x-small fab class="my-0" @click="addCourse">
              <v-icon>mdi-check</v-icon>
            </v-btn>
            <v-btn color="error" x-small fab class="my-0 ml-1" @click="setClose">
...
export default {
  computed: {
    ...
    GrantPartTimeRequest: function() {
      const request = this.application?.funding_requests?.find((fr) => fr.request_type_id === 31);
  ...
  methods: {
    ...
    async deleteCourse(id) {
      try {
        const resDelete = await axios.delete(APPLICATION_URL + `/${id}/course`);
    ...
    async updateCourse(itemToUpdate, id) {
      try {
        const resUpdate = await axios.patch(APPLICATION_URL + `/${this.application.id}/course/${id}`, {
          data: { ...itemToUpdate },
        });
