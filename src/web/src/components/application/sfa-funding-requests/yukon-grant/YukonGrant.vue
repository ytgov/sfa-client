<template>
  <div>
    <div class="row  mt-2">
      <v-switch label="" class="my-0 mr-2" v-model="checkYGRequest" @change="toggle($event)"> </v-switch>
...
export default {
  ...
  computed: {
    ...
    yukonGrantRequest: function() {
      const ygRequest = this.application?.funding_requests?.find((fr) => fr.request_type_id === 2);
