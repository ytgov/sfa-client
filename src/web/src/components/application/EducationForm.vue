<template>
  <div class="home">
    ...
    <v-card class="default mt-6">
      <v-card-text>
        <h1 class="text-h6 font-weight-regular">Primary Address</h1>
        <v-select
          outlined
          dense
          background-color="white"
          ...
          v-model="application.primary_address_id"
          item-text="address_display"
          item-value="id"
          @change="doSaveApp('primary_address_id', application.primary_address_id)"
        ></v-select>
      </v-card-text>
    </v-card>
      ...
      <v-card-text>
        ...
        <div class="row">
          ...
          <div class="col-md-2 d-flex justify-end">
            <v-btn color="success" class="my-0" @click="showPDF">
              View HST
            </v-btn>
          </div>
        </div>
        <div class="row">
          ...
          <div class="col-md-2">
            <v-select
              ...
              @change="
                doSaveStudent('high_school_left_month', student.high_school_left_month, 'studentInfo', student.id)
              "
              :items="monthOptions"
            >
            </v-select>
...
export default {
  ...
  methods: {
    ...
    async showPDF() {
      try {
        let buf = await fetch(
          APPLICATION_URL + `/${this.application.id}/student/${this.student.id}/files/1`
